...
| **L-1 … L-13** | — | — | Leads (high-signal trails, not fully exploited) — see [Leads](#leads) and [X-Ray](#-pre-audit-x-ray-audit-readiness) | 🟢 Resolved — L-1/L-2/L-4/L-6/L-8/L-13 fixed in code; L-3/L-5/L-7/L-9/L-10 documented (verification/operational); L-11/L-12 testnet-only (open) |
...
- **L-1 · `type(uint48).max` "deactivated" delay reverts instead of returning false** — `SyncTrigger._getAmountToSync` *(2 agents)*. ✅ **Fixed (uncommitted).** `_getAmountToSync` now widens to `uint256(_lastExecution) + _delay` (`SyncTrigger.sol:175`), so the deactivated max-delay threshold is simply unreachable and `shouldSync()`/`getAmountToSync()`/`triggerSync()` return a clean "no sync" (0) instead of an overflow panic the CRE `eth_call` probe would misread as a revert. Test updated: `test_shouldSync_falseWhenDeactivated`. *Original analysis:* `_lastExecution + _delay` overflowed `uint48` when `_delay == type(uint48).max`. Inherited verbatim from upstream `SyncAutomation` (which is **not known to be audited** — Chainlink's Direct Staking template docs state it "has not been audited"; see `docs/audit-scope.md` §2).
- **L-2 · Two decoders disagree on `feeOtoD` length domain** — `SyncTrigger.setFeeOtoD` / `triggerSync`. ✅ **Fixed (uncommitted).** `_setFeeOtoD` now validates with `FeeCodec.decodeCCIP` (length must be 21) and `_setFeeDtoO` with `FeeCodec.decodeFee` (length ≥ 17) — exactly the decoders `CustomSender._ccipBuildAndSend` re-runs downstream — so a bad-length config reverts at set-time instead of self-DoSing inside `sync`. Tests added: `test_setFeeOtoD_revertsOnWrongLength`, `test_setFeeDtoO_revertsOnTooShort`. *Original analysis:* `decodeFeeMemory` accepted any ≥17-byte buffer but `decodeCCIP` requires exactly 21, so a 17–20-byte feeOtoD passed `SyncTrigger` then reverted in `sync`.
- **L-3 · `onReport` correctness coupled to one unpinned forwarder version** — `CREReceiver.onReport` / `_extractWorkflowOwner`. 🟢 **Documented (verification).** The repo vendors two Keystone forwarders with incompatible `onReport`/metadata ABIs; `CREReceiver` assumes the CCIP `onReport(bytes,bytes)` + `metadata[42:62]` layout. A pre-live check now confirms `L2_CRE_FORWARDER.typeAndVersion() == "Forwarder and Router 1.0.0"` (RUNBOOK §1 / `docs/audit-scope.md` §3), rejecting the legacy `"KeystoneForwarder 1.0.0"`. *Original analysis retained below.* The forwarder is set from `L2_CRE_FORWARDER` env with no on-chain version assertion in the contract.
- **L-4 · No explicit `metadata.length` guard before slicing** — `CREReceiver._extractWorkflowOwner` *(2 agents)*. ✅ **Fixed (uncommitted).** `_extractWorkflowOwner` now asserts `metadata.length >= 62` (new error `MetadataTooShort`, `CREReceiver.sol:168`) before slicing `metadata[42:62]`, so a malformed-length buffer fails with a named error instead of an implicit calldata-bounds panic. Test added: `test_onReport_revertsOnShortMetadata`. *Original analysis:* relied on Solidity's implicit slice bounds-revert — safe given the fixed Keystone layout + trusted forwarder, but brittle if the metadata format changes.
- **L-5 · `triggerSync` funds the CCIP fee from SyncTrigger's own balance with no refill path** — `SyncTrigger.triggerSync`. 🟢 **Documented (operational).** This is an intended liveness dependency: the trigger fronts each sync's native fee from its own balance and is monitored (`SyncTrigger` balance ≥ 2× `getMaxFees().maxNativeFee`, depletes ~0.005 ETH/sync), topped up via `receive()`, and recovered via owner-only `sweep()` — see RUNBOOK §3 Watch + README §Funding the float. *Original analysis:* `CREReceiver` calls `target.call(data)` with zero value, so once the trigger's balance drops below `nativeAmount` every sync reverts until topped up.

### L2 migration / orchestration (Foundry scripts, trusted-operator)

- **L-6 · Stage-2 grants `SYNC_ROLE` without re-asserting the trigger's wiring** — `L2UpgradeActions.executeMigrationSteps`. ✅ **Fixed (uncommitted).** `_assertSyncInfrastructure` now asserts `ISyncTrigger(syncTrigger).SENDER() == cfg.customSender` (alongside the existing forwarder/owner checks), and `executeMigrationSteps` runs that full assertion as a precondition (see L-8) before granting `SYNC_ROLE` — so an `L2_SYNC_TRIGGER` typo / mis-wired trigger is rejected. *Original analysis:* post-conditions checked the grant but never `SENDER()`/`getForwarder()`, so a wrong trigger could be granted `SYNC_ROLE` and brick sync.
- **L-7 · `SYNC_ROLE` "sole holder" invariant is unverifiable on-chain** — `L2UpgradeActions.executeMigrationSteps`. 🟢 **Documented (verification).** `CustomSender` inherits non-enumerable `AccessControlUpgradeable`, so this cannot be asserted on-chain; instead state-mate reconstructs the **complete** `SYNC_ROLE` member set from event logs and asserts `count == 1` (RUNBOOK G4 table). *Original analysis:* the script revokes only ≤2 hardcoded legacy automations, so any pre-existing extra holder would retain `CustomSender.sync()` after migration unless caught off-chain.
- **L-8 · No on-chain Stage-1-completeness precondition before irreversible handover** — `L2UpgradeActions.executeMigrationSteps`. ✅ **Fixed (uncommitted).** `executeMigrationSteps` now calls `_assertSyncInfrastructure` as its **first** statement — asserting the full SyncTrigger/CREReceiver wiring (SENDER/forwarder/owner, expectedAuthor, allow-list seed, fee float) — before `setOraclePool` / `SYNC_ROLE` grant / admin revoke / ProxyAdmin handover, so a half-configured Stage 1 reverts before any irreversible write. Threaded `creReceiver`/`creForwarder` through all callers (env reads in `runMigrate`/Sepolia). Test added: `test_executeMigrationStepsRevertsOnMiswiredStage1` (×4 networks). *Original analysis:* `runVerifyStage1` was a separate optional view; `runMigrate` didn't require it.
- **L-9 · Cross-chain mirror invariant `expectedAuthor (L2) == WorkflowRegistry.owner (L1)` is never reconciled** — `CREReceiver.expectedAuthor` / `verifyStage1`. 🟢 **Documented (verification).** Reconciled via two *independent on-chain* reads: the `deploy-cre-workflow` recipe reads live L2 `CREReceiver.getExpectedAuthor()` and aborts if the registered workflow owner ≠ that pin; `VerifyCREWorkflow` reads L1 `WorkflowRegistry.getWorkflowById(id).owner`; gate **G2-author** additionally requires an observed `CallExecuted` from the live DON. Manual `VerifyCREWorkflow` runs must source `CRE_EXPECTED_AUTHOR` from the on-chain L2 pin, not the tautological `L2_LIQUIDITY_OWNER` env fallback (RUNBOOK §2/§3, `docs/audit-scope.md` §3). *Original analysis retained.*
- **L-10 · `setOraclePool` repoints `CustomSender` to a freshly-deployed, unfunded pool** — `L2UpgradeActions.executeMigrationSteps`. 🟢 **Documented (operational).** RUNBOOK §3 Finalize makes it a duty: the LOL multisig seeds wstETH into each new pool post-G4, and "until seeded, `fastStake` reverts" (`sync`/slowStake unaffected). *Original analysis:* mainnet lanes have no on-chain liquidity-migration step, so `fastStake` reverts (`OraclePoolInsufficientTokenOut`) until the out-of-band LOL funding tx lands.
...
- **CCIP fee / native / LINK accounting** in `SyncTrigger.triggerSync` → `CustomSender.sync` → `_ccipBuildAndSend` is byte-for-byte the upstream `SyncAutomation` (logic-equivalent; **upstream not known to be audited** — see `docs/audit-scope.md` §2); over-funded native is refunded via `TokenHelper.refundExcessNative(msg.sender)`. No over/under-send, no stranded value, no native/LINK confusion. *(math-precision, economic-security, execution-trace)*
...
|-----|--------------------------------|---------|
| **I-2** | ✅ Now enforced — deactivated delay (`type(uint48).max`) returns "no sync"; `uint256(_lastExecution) + _delay` no longer overflows | L-1 (fixed) |
| **I-4** | ✅ Now enforced — `SyncTrigger._setForwarder` rejects `address(0)`, aligned with `CREReceiver` | **L-13** (fixed) |
...
- **L-13 · `SyncTrigger.setForwarder` accepts `address(0)`** — `SyncTrigger.sol:148,186` — ✅ **Fixed (uncommitted).** `_setForwarder` now reverts on `address(0)` (new error `SyncTriggerInvalidForwarder`, `SyncTrigger.sol:186`), matching `CREReceiver.setForwarder`. Test updated: `test_setForwarder_revertsOnZero`. *Original smell:* a single owner call to `setForwarder(0)` would permanently brick `triggerSync` (its `onlyForwarder` compares `msg.sender != address(0)`, always true). Owner-only self-brick, not externally exploitable.
